function [ G_ori, G_normRef] = gen_stochgeom_channel_matrix_fixedKL(path_loss, side_length, N_u, N_r, numusers, numrelays)
% Inputs: number of users, number of relays, path loss exponent, side length of
% square area
...
xusers = side_length*rand(1, numusers); 
yusers = side_length*rand(1, numusers); 
xrelays = side_length*rand(1, numrelays); 
yrelays = side_length*rand(1, numrelays); 
...
G_ori = zeros(N_r*L, N_u*K); % original channel matrix
G_Ref = zeros(N_r*L, N_u*K); % free space channel matrix with blockage ...
                     % and shadowing according to [1]
G_UMi = zeros(N_r*L, N_u*K); % 3GPP UMi channel matrix with blockage and shadowing
G_normRef = zeros(N_r*L, N_u*K);
G_normUMi = zeros(N_r*L, N_u*K);
...
for kk = 1:K
    for ll = 1:L
        ...
        if (bool_LOS == true)
            %  fast-fading envelope of the LOS links: Nakagami-m dist.
            %  parameters: m = 2, omega_LOS = 1
            rv_LOS = random(pd_LOS, [N_r, N_u]);
            EnvFF_dB = 10 * log10(rv_LOS); % Envelope for FF in dB 
        else % bool_LOS == false
            %  fast-fading envelope of the NLOS links: Rayleigh dist.
            %  parameters: omega_NLOS = 1
            rv_NLOS = random(pd_NLOS, [N_r, N_u]);
...
%            ((yusers(kk) - yrelays(ll))^2))^(-path_loss/2);
        G_ori(((ll-1)*N_r + 1):(ll*N_r), ((kk-1)*N_u + 1):(kk*N_u)) = ...
            (max(dist_ori, dist_threshold)/side_length_meter)^-path_loss;
        % channel components generated according to path loss exponent;
        % given user locations, no extra randomness in channel 
        
        G_Ref(((ll-1)*N_r + 1):(ll*N_r), ((kk-1)*N_u + 1):(kk*N_u)) = ...
            power(10, -1*RefPL_dB/10); 
        G_normRef(((ll-1)*N_r + 1):(ll*N_r), ((kk-1)*N_u + 1):(kk*N_u)) = ...
            power(10, -1*normRefPL_dB/10); 
        G_UMi(((ll-1)*N_r + 1):(ll*N_r), ((kk-1)*N_u + 1):(kk*N_u)) = ...
            power(10, -1*UMiPL_dB/10);
    end
end
